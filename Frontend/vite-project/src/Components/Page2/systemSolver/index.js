const buildAugmentedMatrix = (leftMatrix, rightMatrix) => 
  leftMatrix.map((row, i) => row.concat(rightMatrix[i]));
...
const solve = (leftMatrix, rightMatrix) => 
  backSubstitute(triangularize(buildAugmentedMatrix(leftMatrix, rightMatrix)));

// Exporturi moderne
export { buildAugmentedMatrix, triangularize, backSubstitute, solve };
